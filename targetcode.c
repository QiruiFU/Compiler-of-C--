...
void TgtCodeList(InterCodeList *CodeList, FILE *fout){
    init_tar(fout);
    InterCodeNode *code = CodeList->head_;
    while(code != NULL){
        if(code->node_.kind == FUNC){
            tot_offset = 4;
            my_alloc(code);
        }
        TgtCode(code->node_, fout);
        code = code->next;
    }
}
...
void my_alloc(InterCodeNode *code){
    ...
    while(cur!=NULL && cur->node_.kind!=FUNC){
        switch(cur->node_.kind){
            case ADD:
            case SUB:
            case MULTI:
            case DIV:
                AddrVar(cur->node_.u.op1);
                AddrVar(cur->node_.u.op2);
                AddrVar(cur->node_.u.op3);
                break;
            case ASSIGN_VAL2VAL:
            case ASSIGN_VAL2PNT:
            case ASSIGN_PNT2VAL:
            ...
            case DEC:
                {
                    ...
                    p->offset = tot_offset;
                    p->op = cur->node_.u.op1;
                    p->nxt = VariListHead;
                    VariListHead = p;
                    tot_offset += cur->node_.u.op2.u.val_const;
                }
                break;
            case ARG:
                AddrVar(cur->node_.u.op1);
...
void TgtFunc(InterCode code, FILE *fout){
    fprintf(fout, "\n%s:\n", code.u.op1.u.func_name);
    strcpy(cur_func, code.u.op1.u.func_name);
    if(strcmp(cur_func, "main")==0){
...
void TgtDec(InterCode code, FILE *fout){
    ...
    p->nxt = VariListHead;
    VariListHead = p;
    tot_offset += 4 * code.u.op2.u.val_const;
}
